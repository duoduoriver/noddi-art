...
Usage:
  python scripts/fetch_og_image.py "https://example.com" "public/og.png"
...
def _fetch_bytes(url: str, *, timeout: int = 20) -> tuple[bytes, str]:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 (compatible; tanstack-quick-start/1.0)",
...
def main() -> int:
    if len(sys.argv) != 3:
        print(
            "Usage: python scripts/fetch_og_image.py <url> <output-path>",
            file=sys.stderr,
        )
        return 2
