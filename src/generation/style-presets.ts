...
export function stylePromptDirection(style: unknown) {
  ...
  return (
    ...
    (raw
      ? `Use the requested ${raw} visual style as a strong design direction. Keep it appropriate for a clean, recognizable app icon and avoid unnecessary visual complexity.`
      : 'Use a clean, recognizable app-icon visual style with a strong silhouette and controlled detail.')
  );
}
