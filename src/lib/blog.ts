...
export type BlogPostSummary = Pick<
  BlogPost,
  'category' | 'date' | 'description' | 'image' | 'slug' | 'title'
>;
...
export function toPostDetail(post: BlogPost): BlogPost {
  return post;
}

function toPostSummary(post: BlogPost): BlogPostSummary {
  const { category, date, description, image, slug, title } = post;
  return { category, date, description, image, slug, title };
}
...
export function getPaginatedPosts(page: number): {
  ...
  return {
    posts: sorted.slice(start, start + pageSize).map(toPostSummary),
    totalPages,
    currentPage,
  };
}
