...
The project uses **T3 Env** (`@t3-oss/env-core`) for build/server-side
validation, while browser code reads a lightweight public env object:

| Source | File | When | Prefix / source |
|--------|------|------|------------------|
| **publicEnv** | `src/env/public.ts` | Client bundle (inlined by Vite) | `VITE_*` from `import.meta.env` |
| **validateClientEnv** | `src/env/client.ts` | Vite config/build startup | `VITE_*` loaded with `loadEnv()` |
| **serverEnv** | `src/env/server.ts` | Runtime (Worker) | `process.env` (Wrangler vars/secrets) |

**Rule of thumb:** Use **publicEnv** in app code for values inlined at build
(public URLs, analytics IDs, feature keys). Keep validation in
**validateClientEnv** so `@t3-oss/env-core` and Zod do not get pulled into the
marketing client bundle. Use **serverEnv** for secrets and server-only config
(API keys, webhook secrets). Deployment target is **Cloudflare Workers**.

---

## 1. Build-time public env
...
Validation runs from `vite.config.ts` via `validateClientEnv(loadEnv(...))`.
Client-shipped modules should import `publicEnv` from `src/env/public.ts`, not
`src/env/client.ts`.
...
`getBaseUrl()` in `src/lib/urls.ts` reads **publicEnv.VITE_BASE_URL**
(build-time value inlined by Vite).
