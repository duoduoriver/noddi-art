...
1. **Create the R2 bucket** (once per environment):
   ...
   Use the same name as `bucket_name` in `wrangler.jsonc` (e.g. `duoduohe-tanstack`).
...
```
src/storage/
├── index.ts           # uploadFile, deleteFile, getFile, provider registry/helpers
