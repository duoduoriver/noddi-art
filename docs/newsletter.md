...
```
src/newsletter/
├── index.ts           # subscribe, unsubscribe, isSubscribed plus internal provider registry
...
| **Newsletter provider** | Built internally from config + env (no caching; new instance per call). |
